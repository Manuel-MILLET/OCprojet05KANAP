...
function removeItemProduct(){
	// traitement des suppressions d'item des produits selectionnés
	console.log('fonction remouve item en route');
	for (let i in cartUser){
		const element = document.getElementsByClassName("deleteItem")[i];
		const paragrapheCible = element.closest('.deleteItem');
		paragrapheCible.addEventListener("click", function() {
			console.log('Bravo tu as cliqué sur le paragraphe n°'+Number(i+1)+'  ');
		});
	}
}
*/
